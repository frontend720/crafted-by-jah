...
export default function Chat() {
  ...
  return (
    ...
    >
      ...
      <div style={toggleModal ? {display: ""} : {display: "none"}} className="modal">
        <button className="modal-button" onClick={isModalVisible}>Close</button>
      <ReactMarkdown children={interpretation} remarkPlugins={[remarkGfm]} />
      </div>
      ...
      >
        <div className="preview-flex">
          ...
          <img style={url === "" ? {display: "none"} : {display: ""}} onClick={imageSize} src={url} width={size ? "25px" : "100px"} alt="" />
          <FaInfo size="20px" style={imageToggle ? {display: "", marginLeft: 10} : {display: "none"}} onClick={isModalVisible}/>
        </div>
        <TextareaAutosize
          maxRows={4}
          className="textarea"
        ...
        {chatHistory.map((item, index) => (
          <div key={index} style={{ marginBottom: "10px" }}>
            <p className="prompt">
              ...
              <strong className="participants">Tariq:</strong>
